...
from .types import ProviderMaps, EnvMap, RuntimeContext
...
def resolve_injector(
    ...
    if injector.when:
        if not _evaluate_condition(injector.when, context, providers, token_engine):
...
def _evaluate_condition(
    ...
    from .expression_parser import evaluate_expression, ExpressionError
...
def _coerce_type(value: str, target_type: str, injector: Optional[Injector] = None) -> tuple[Optional[str], List[str]]:
...
def _create_temp_file(content: str, injector: Injector) -> Path:
    ...
    suffix = '.tmp'
    if injector.type == 'json':
        suffix = '.json'
    elif injector.type == 'path':
        # Try to infer suffix from content
